...
class WallStage(PipelineStage):
    ...
    def run(self, context: PipelineContext) -> StageStatus:
        ...
        if (
            ...
            raise RuntimeError("Steps 0-4 must run before wall fitting")

        print("[step 5] fitting global wall planes...")
        context.walls = fit_wall_planes(
            context.point_map,
            context.mask_pm,
            context.floor,
        )
        print(f"[step 5] detected {len(context.walls)} wall plane(s)")
